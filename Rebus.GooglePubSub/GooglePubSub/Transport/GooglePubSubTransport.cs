...
namespace Rebus.GooglePubSub.Transport
{
    /// <summary>
    /// Implementation of <see cref="ITransport"/> that uses Google Pub/Sub to do its thing
    /// </summary>
    public class GooglePubSubTransport : ITransport, IInitializable
    {
        ...
        readonly GooglePubSubTransportOptions _options;
        ...
        /// </summary>
        public GooglePubSubTransport(ICloudQueueFactory queueFactory, string inputQueueName, IRebusLoggerFactory rebusLoggerFactory, GooglePubSubTransportOptions options, IRebusTime rebusTime)
        {
            if (rebusLoggerFactory == null) throw new ArgumentNullException(nameof(rebusLoggerFactory));
        ...
        public async Task<TransportMessage> Receive(ITransactionContext context, CancellationToken cancellationToken)
        {
            if (Address == null)
            {
                throw new InvalidOperationException("This Google Pub/Sub transport does not have an input queue, which means that it is configured to be a one-way client. Therefore, it is not possible to receive anything.");
            }
        ...
        public void Initialize()
        {
            if (Address != null)
            {
                _log.Info("Initializing Google Pub/Sub transport with queue '{0}'", Address);
                CreateQueue(Address);
                return;
            }

            _log.Info("Initializing one-way Google Pub/Sub transport");
        }
